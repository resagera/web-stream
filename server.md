...
Если `guests.json`, `invites.json` или `event.json` невозможно распарсить как JSON при старте, файл переименовывается в `*.corrupt.<timestamp>`, а сервер создает новый файл с пустым/default состоянием. Для `chat.jsonl` и `journal.jsonl` отдельные битые строки пропускаются; если ломается само чтение файла, он тоже уходит в `*.corrupt.<timestamp>`.

`guests.json`, `invites.json` и `event.json` записываются атомарно: данные пишутся во временный файл в той же директории, синхронизируются на диск и затем заменяют целевой файл через `rename`.
...
broadcaster   BROADCASTER_PASSWORD
