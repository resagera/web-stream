...
- сделать более полный admin API для управления событием.
...
Проверить LiveKit-подключение с реальной камерой в браузере и новый экран устройства-транслятора на реальном телефоне/ноутбуке. После этого проверить TURN на реальном домене с DNS и сертификатом.
